...
import { safeStageErrorCode } from '../../ai/src/contracts/stage-error.js';
...
export class PostgresPersistence implements WorkflowPersistence {
 constructor(private readonly pool:Pool, private readonly ownerRun?: {id:string;scope:PersistenceScope}) {}
 ...
 private async authorize(db:PoolClient,s:PersistenceScope,write:boolean):Promise<void>{
  if(this.ownerRun){
   const allowed=this.ownerRun.scope;
   if(s.actorId!==allowed.actorId||s.projectId!==allowed.projectId||s.organizationId!==allowed.organizationId)throw new PersistenceError('ACCESS_DENIED');
   const run=await db.query(`SELECT id FROM kleo.workflow_runs WHERE id=$1 AND actor_id=$2 AND organization_id=$3 AND project_id=$4 AND source_brief_version_id IS NOT NULL`,[this.ownerRun.id,s.actorId,s.organizationId,s.projectId]);
   if(!run.rows.length)throw new PersistenceError('ACCESS_DENIED');return;
  }
 ...
 private async runRow(db:PoolClient,s:PersistenceScope,id:string,lock=false){
  if(this.ownerRun&&id!==this.ownerRun.id)throw new PersistenceError('ACCESS_DENIED');
  const result=await db.query(`SELECT id,status,result_digest,actor_id,source_brief_version_id FROM kleo.workflow_runs WHERE organization_id=$1 AND project_id=$2 AND id=$3${lock?' FOR UPDATE':''}`,[s.organizationId,s.projectId,id]);
 ...
 async finishRun(scope:PersistenceScope,runId:string,value:WebsiteWorkflowResult,options:{websiteId?:string;cancelled?:boolean;failureCode?:'BUDGET_EXCEEDED'|'TIMEOUT'}={}):Promise<StoredRun>{
  const s=scopeCopy(scope);uuid(runId);const opts=snapshot(options);if(Object.keys(opts).some(k=>!['websiteId','cancelled','failureCode'].includes(k)))invalid();
  ...
  return this.transaction(async db=>{
   ...
   // Rebuild from the exact persisted Brief binding, never from result.state/business.
   const brief=row.source_brief_version_id?(await db.query(`SELECT b.id,b.document FROM kleo.workflow_runs r JOIN kleo.project_briefs b
    ON b.id=r.source_brief_version_id AND b.project_id=r.project_id AND b.organization_id=r.organization_id
    WHERE r.id=$1 AND r.project_id=$2 AND r.organization_id=$3`,[runId,s.projectId,s.organizationId])).rows[0]:undefined;
   if(row.source_brief_version_id&&!brief)invalid();
   const confirmedBusinessFacts=brief?confirmedFactsFromBrief(parseBrief(brief.document),brief.id):undefined;
   let r:WebsiteWorkflowResult;try{r=validateResult(value,s.projectId,confirmedBusinessFacts);}catch{throw new PersistenceError('INVALID_INPUT');}
  if(opts.failureCode!==undefined&&(!['BUDGET_EXCEEDED','TIMEOUT'].includes(String(opts.failureCode))||!this.ownerRun||r.success))invalid();
  if(this.ownerRun&&opts.websiteId)invalid();
  ...
  const stageErrorCode=next!==undefined&&r.stageError?.stage===next?safeStageErrorCode(r.stageError.errorCode):undefined;
  const executions=stages.filter(stage=>outputs.includes(stage)||r.executions?.[stage]!==undefined||!r.success&&!r.state.qa&&stage===next).map(stage=>({stage,status:outputs.includes(stage)?'completed':opts.cancelled?'cancelled':'failed',attempts:telemetry(r.executions?.[stage],s,runId,stage),...(!opts.cancelled&&stage===next&&stageErrorCode?{errorCode:stageErrorCode}:{})}));
  ...
  const hash=digest({state:r.state,status,executions,websiteId:opts.websiteId??null,...(opts.failureCode?{failureCode:opts.failureCode}:{})});
   ...
   for(const e of executions)await this.persistExecution(db,s,runId,e);
   await db.query('UPDATE kleo.workflow_runs SET status=$4,result_digest=$5,completed_at=now(),failure_code=$6 WHERE organization_id=$1 AND project_id=$2 AND id=$3',[s.organizationId,s.projectId,runId,status,hash,status==='failed'?(opts.failureCode??'WORKFLOW_FAILED'):status==='cancelled'?'CANCELLED':null]);
   await this.audit(db,s,runId,`workflow_${status}`);return this.summary(db,s,{id:runId,status});
  });
 }
 private async persistExecution(db:PoolClient,s:PersistenceScope,runId:string,e:{stage:string;status:string;errorCode?:string;attempts:ReturnType<typeof telemetry>}) {
   const previous=(await db.query('SELECT id,status FROM kleo.agent_executions WHERE workflow_run_id=$1 AND agent_type=$2',[runId,e.stage])).rows[0];
   if(previous){if(previous.status!==e.status)throw new PersistenceError('CONFLICT');return;}
   const executionId=randomUUID();
    await db.query('INSERT INTO kleo.agent_executions(id,organization_id,project_id,workflow_run_id,agent_type,status,error_code) VALUES($1,$2,$3,$4,$5,$6,$7)',[executionId,s.organizationId,s.projectId,runId,e.stage,e.status,e.status==='completed'?null:e.status==='cancelled'?'CANCELLED':safeStageErrorCode(e.errorCode)??'STAGE_FAILED']);
 ...
 async stage(scope:PersistenceScope,runId:string,stage:typeof stages[number],phase:'started'|'completed',execution?:import('../../ai/src/agent.js').AgentResult['execution']) {
  const s=scopeCopy(scope);uuid(runId);if(!this.ownerRun||!stages.includes(stage))invalid();
  await this.transaction(async db=>{await this.authorize(db,s,true);const row=await this.runRow(db,s,runId,true);if(row.status!=='running')throw new PersistenceError('CONFLICT');
   if(phase==='started')await db.query('UPDATE kleo.workflow_runs SET current_stage=$2 WHERE id=$1',[runId,stage]);
   else await this.persistExecution(db,s,runId,{stage,status:'completed',attempts:telemetry(execution,s,runId,stage)});
  });
 }
 ...
 async listRuns(scope:PersistenceScope,limit=50):Promise<StoredRun[]>{const s=scopeCopy(scope);if(!Number.isInteger(limit)||limit<1||limit>100)invalid();return this.transaction(async db=>{await this.authorize(db,s,false);if(this.ownerRun)return [await this.summary(db,s,await this.runRow(db,s,this.ownerRun.id))];const rows=await db.query('SELECT id,status FROM kleo.workflow_runs WHERE organization_id=$1 AND project_id=$2 ORDER BY started_at DESC,id DESC LIMIT $3',[s.organizationId,s.projectId,limit]);return Promise.all(rows.rows.map(r=>this.summary(db,s,r)));});}
 async getVersion(scope:PersistenceScope,id:string):Promise<{id:string;websiteId:string;workflowRunId:string;versionNumber:number;document:unknown;qa:unknown|null}>{
  const s=scopeCopy(scope);uuid(id);return this.transaction(async db=>{await this.authorize(db,s,false);
   const result=await db.query(`SELECT v.id,v.website_id,v.workflow_run_id,v.version_number,v.document,q.document AS qa FROM kleo.website_versions v
    ...
    WHERE v.organization_id=$1 AND v.project_id=$2 AND v.id=$3 AND ($4::uuid IS NULL OR v.workflow_run_id=$4)`,[s.organizationId,s.projectId,id,this.ownerRun?.id??null]);
