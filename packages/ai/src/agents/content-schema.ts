...
export function validateContentInput(value:unknown):ContentAgentInput {
  ...
  if(!value||typeof value!=='object'||Array.isArray(value)||Object.keys(value).some(k=>!['business','design','businessFacts','confirmedBusinessFacts'].includes(k))) throw new SecurityError('INVALID_INPUT');
...
export function buildContentWireSchema(desiredActions:readonly string[]):Record<string,unknown> {
  ...
  const variants=[];
  for(const kind of ['regular','faq','cta']) {
    if(kind==='cta'&&!actions.length) continue;
    const properties={...base.properties,
      type:{type:'string',enum:kind==='regular'?contentSectionSchema.properties.type.enum.filter(t=>t!=='faq'&&t!=='cta'):[kind]},
