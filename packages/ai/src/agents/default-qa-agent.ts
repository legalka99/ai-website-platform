...
import { snapshotQAInput,deterministicQA,qaWireSchema,parseQAWire,mergeQAFindings,QA_AI_CODES } from './qa-schema.js';
...
export class DefaultQAAgent implements QAAgent {
  ...
  #router:AIRouter;
  #projectId:string;
  constructor(router:AIRouter,projectId:string) {
  ...
  async run(context:AgentContext<QAAgentInput>):Promise<AgentResult<QAReport>> {
    ...
    try {
      ...
      const findings=deterministicQA(input,context.projectId);
      ...
      const data=untrustedDataMessage({goal:context.goal,reviewContext:input.reviewContext,website:{...site,pages:site.pages.map(({id,...p})=>({...p,blocks:p.blocks.map(({id,...b})=>b)}))}});
      if(data.content.length>18000)throw new SecurityError('INVALID_INPUT');
      const response=await this.#router.generate({model:'route',messages:[{role:'system',content:QA_INSTRUCTIONS},data],structuredOutput:{name:'qa_report',schema:qaWireSchema},signal:context.signal,context:{projectId:context.projectId,goal:context.goal}});
      ...
      const parsed=parseQAWire(wire,input);validationError=parsed.validationError;
