...
export type BusinessProviderConfig = ({id:'openai';config:Omit<OpenAIProviderConfig,'apiKey'>}|{id:'yandex';config:Omit<YandexProviderConfig,'apiKey'>}) & {
...
export async function createRoutedBusinessService(options:{context:AIServiceContext;authorization:AuthorizationPolicy;costs:AICostGuard;secrets:SecretProvider;
  providers:readonly BusinessProviderConfig[];policy:RouterPolicy;health?:ProviderHealthTracker}):Promise<DefaultBusinessAgent> {
  ...
  const configs=options.providers.map(p=>({...p,config:{...p.config},credentials:{...p.credentials},capabilities:p.capabilities?{...p.capabilities}:undefined})) as BusinessProviderConfig[];
  ...
  for(const p of configs) {
    ...
    bindings.push({metadata:{id:p.id,model,capabilities},provider:new GuardedAIProvider(adapter,context,options.authorization,options.costs,capabilities.maxOutputTokens)});
  }
  return new DefaultBusinessAgent(new AIRouter(bindings,policy,'business',options.health));
}
