...
export function parseContentSmokeArgs(args) {
...
export function contentSmokeInput() {
  return {business:{companyName:'Учебный пример Kleo',industry:'Стеклянные конструкции',
    ...
    design:{styleName:'Минималистичный',description:'Спокойная страница с короткими понятными блоками',mood:['Спокойный','Визуально лёгкий'],
      colors:{primary:'#334455',background:'#ffffff',text:'#111111'},typography:{headingStyle:'Простой шрифт без засечек',bodyStyle:'Читаемый шрифт без засечек'},
      layoutPrinciples:['Короткие разделы','Заметное действие для запроса расчёта']}};
}
...
export function contentSmokeOutput(result,context,provider,model,knownSecrets=[]) {
  const execution=result.execution;
  const routing=execution?.routing;
  const decision=routing?.decision;
  const valid=result.success===true && validateContentPlan(result.output).valid;
  const data={success:valid,projectId:context.projectId,workflowId:context.workflowId,provider,model,
    ...(valid?{content:structuredClone(result.output)}:{error:'Content smoke не вернул проверенный результат.',...strings(result,['errorCode'])}),
    ...(!valid&&safeContentValidationError(result.validationError)?{validationError:safeContentValidationError(result.validationError)}:{}),
...
export async function runContentSmoke(args,{loadEnvironment=loadLocalEnvironment,transport,write=message=>console.log(message)}={}) {
  let parsed;
  try {parsed=parseContentSmokeArgs(args);} catch {write('Некорректные параметры. Допустимы --provider=openai или --provider=yandex и --confirm-paid-request.');return 1;}
  ...
  try {
    ...
    const context={projectId:'kleo-content-smoke',organizationId:'local',workflowId:`content-${randomUUID()}`,actor:{id:'local-owner',authenticated:true}};
    ...
    const agent=await createRoutedContentService({context,policy,costs,
    ...
    const result=await agent.run({projectId:context.projectId,goal:'Подготовить компактный контентный план главной страницы для запроса расчёта.',input:contentSmokeInput()});
    write(contentSmokeOutput(result,context,parsed.provider,config.model,[apiKey]));
    return result.success?0:1;
  } catch {write('Не удалось запустить Content smoke. Проверьте локальную конфигурацию, режим и права файла настроек.');return 1;}
}
if(process.argv[1] && import.meta.url===pathToFileURL(process.argv[1]).href) process.exitCode=await runContentSmoke(process.argv.slice(2));
