...
export async function loadMigrations(){const dir=new URL('../packages/persistence/migrations/',import.meta.url);return Promise.all((await readdir(dir)).filter(n=>n.endsWith('.sql')&&n!=='006_content_correction_usage.sql').sort().map(async name=>({name,sql:await readFile(new URL(name,dir),'utf8')})));}
