...
import { validateDeveloperOutput } from '../.test-build/packages/ai/src/validation/developer-output-validator.js';
import { contentSmokeInput } from './smoke-content.mjs';
...
export function parseDeveloperSmokeArgs(args) {
...
export function developerSmokeInput() {
  const input=contentSmokeInput();
  return {...input,content:{pageTitle:'Glass partitions',pageGoal:'Request a quote',toneOfVoice:'Calm',keyMessages:['Glass partitions for apartments'],sections:[
    {type:'hero',purpose:'Introduce the project',heading:'Glass partitions',text:'Describe your project.',callToAction:input.business.desiredActions[0]},
    {type:'services',purpose:'Present the range',heading:'Partitions and shower enclosures'},
    {type:'cta',purpose:'Request project details',heading:'Discuss your project',callToAction:input.business.desiredActions[0]}
  ]}};
}
...
export function developerSmokeOutput(result,context,provider,model,knownSecrets=[]) {
  const execution=result.execution;
  const routing=execution?.routing;
  const decision=routing?.decision;
  const valid=result.success===true && validateDeveloperOutput(result.output,context.projectId).valid;
  const data={success:valid,projectId:context.projectId,workflowId:context.workflowId,provider,model,
    ...(valid?{website:{name:result.output.website.name,status:result.output.website.status,pages:result.output.website.pages.map(p=>({slug:p.slug,title:p.title,blockTypes:p.blocks.map(b=>b.type),blockCount:p.blocks.length}))}}:{error:'Developer smoke не вернул проверенный результат.',...strings(result,['errorCode'])}),
    ...(!valid&&safeDeveloperValidationError(result.validationError)?{validationError:safeDeveloperValidationError(result.validationError)}:{}),
...
export async function runDeveloperSmoke(args,{loadEnvironment=loadLocalEnvironment,transport,write=message=>console.log(message)}={}) {
  let parsed;
  try {parsed=parseDeveloperSmokeArgs(args);} catch {write('Некорректные параметры. Допустимы --provider=openai или --provider=yandex и --confirm-paid-request.');return 1;}
  ...
  try {
    ...
    const context={projectId:'kleo-developer-smoke',organizationId:'local',workflowId:`developer-${randomUUID()}`,actor:{id:'local-owner',authenticated:true}};
    ...
    const agent=await createRoutedDeveloperService({context,policy,costs,
    ...
    const result=await agent.run({projectId:context.projectId,goal:'Собрать draft главной страницы из проверенного контента и направления дизайна.',input:developerSmokeInput()});
    write(developerSmokeOutput(result,context,parsed.provider,config.model,[apiKey]));
    return result.success?0:1;
  } catch {write('Не удалось запустить Developer smoke. Проверьте локальную конфигурацию, режим и права файла настроек.');return 1;}
}
if(process.argv[1] && import.meta.url===pathToFileURL(process.argv[1]).href) process.exitCode=await runDeveloperSmoke(process.argv.slice(2));
