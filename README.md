...
Structured Brief advantages: new Brief versions store up to eight independently owner-confirmed `{text}` items (300 characters each, 1500 total). Historical string values remain immutable and retain conservative legacy atomization until the owner reviews and saves a new version. ConfirmedBusinessFacts records structured provenance as `item:{index,count}`; the deterministic advantages path pins exact owner text without widening qualified, negative or conditional claims. No SQL migration or historical rewrite is used.
...
Safe legacy fact atomization remains only for historical free-form advantages and products/services. Unknown or mixed legacy lists stay one complete fact. Structured advantage items no longer depend on a closed comma-list grammar; each gains authority only through an explicit owner save of a new immutable Brief version.
...
Creative grounding: разделены публичный copy и внутренний план; добавлены ограниченные смысловые перефразы, необязательный стратегический контекст и основа intent/exact-edit. Нового редактора/API нет. [Правила, библиотечный аудит и ограничения](docs/CREATIVE-GROUNDING.md).

**Текущий checkpoint: Block Generation MVP.** Создание одного канонического draft-блока без обязательного Brief, отдельный синхронный workflow с ConfirmedBusinessFacts, owner-only API, immutable версии и раздел «Создание» в Console. Design/Developer переиспользуются детерминированно, Content/QA — через guarded Router. Migration 007 добавлена; migration 006 отложена и не применяется. Revise, preview, page generation и публикация не реализованы. [Архитектура, лимиты, API и ограничения](docs/BLOCK-GENERATION.md). Ранее описанные checkpoint ниже — история.

Grounding: единый server-owned ConfirmedBusinessFacts из конкретной версии Owner Brief передаётся Business/Content и всем validators; persistence заново проверяет evidence по сохранённой привязке. BusinessProfile/industry/цели/CTA не создают коммерческие факты. Условия и отрицания сохраняются; для публичных claims разрешены полные подтверждённые формулировки и ограниченные безопасные перефразы. Внутренние поля не публикуются и не создают factual authority. Content correction отключена по умолчанию; migration 006 отложена и не загружается. Project Knowledge допускает пустой набор фактов и отделён от творческих инструкций и AI suggestions; block editor пока не реализован. [Контракт, ограничения и технический долг](docs/CONFIRMED-BUSINESS-FACTS.md).

**Текущий checkpoint: Owner Workflow Launch MVP.** Из Console владелец запускает существующий Business → Design → Content → Developer → QA pipeline по точной immutable версии брифа. Запуск синхронный, управляемый API-запросом: без очереди, фоновых необслуживаемых Promise и автоматического resume. Записи процесса/этапов/usage переживают обновление браузера; при падении API требуется операторская проверка незавершённого run. Owner-only, CSRF/Origin, идемпотентность, запрет параллельного owner run в проекте, общий server-owned output-token budget. Подробный контракт и ограничения: [ADMIN-CONSOLE](docs/ADMIN-CONSOLE.md#owner-workflow-launch-mvp).
