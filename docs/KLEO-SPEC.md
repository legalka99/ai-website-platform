...
**Structured Owner Brief advantages.** New Brief versions store advantages as an ordered list of up to eight strict `{text}` items (300 characters each, 1500 total). Each explicit owner-saved item becomes one exact ConfirmedBusinessFact with immutable `item:{index,count}` provenance. Historical `string|null` documents remain readable and unchanged; mixed legacy strings are not silently split. Opening and saving an old Brief creates a new structured version and is the explicit authority transition. No SQL migration is required because the immutable document is JSONB.

**Deterministic advantages pinning.** `task.blockType` and normalized intent resolve to one requested type used by provider schema, prompt DATA and final validation. A complete structured owner-item group, or a complete safely atomized legacy fragment group, supplies exact server-owned points after schema/security/CTA validation and before grounding. Qualified, conditional and negative structured entries remain complete cards and never authorize shorter claims. Optional heading/text rejected by grounding are omitted and a rejected required block title is replaced with «Преимущества»; final validation is unchanged.

Block quality refinement: grounding correction now prioritizes available confirmed atomic advantages and uses neutral fallback per card only. Server-selected isolated Block QA omits page SEO data/checks and excludes SEO_INVALID; whole-site QA retains its existing SEO behavior. No extra generation, migration or factual-authority change.

Safe fact atomization remains a backward-compatible adapter for historical free-form advantages/productsOrServices only. Unknown/qualified legacy lists remain intact; structured advantages use item provenance created by an explicit owner save. Authority is reconstructed server-side from the exact saved Brief version. No LLM extraction or security-policy relaxation.

Block Content correction: one new generation after content-grounding failure only. Maximum two Content generations; shared BLOCK_LIMITS stays at four provider requests including QA/fallback. Both generations retain telemetry; rejected output is never replayed. Whole-site correction remains disabled by default.

**Creative grounding checkpoint.** Public Content claims проверяются отдельно от внутренних notes/purpose/pageGoal/toneOfVoice/keyMessages. Разрешён ограниченный полный semantic match с сохранением условий, чисел, отрицаний и scope; ConfirmedBusinessFacts остаётся единственным factual authority. Market/competitor/SEO context — только стратегия. Intent normalization и чистый exact-edit preflight подготовлены без нового API/редактора. Router, correction defaults и migrations не меняются. [Архитектура и ограничения](CREATIVE-GROUNDING.md).

**Текущий checkpoint: Block Generation MVP.** Создание одного канонического draft-блока без обязательного Brief, отдельный синхронный workflow с ConfirmedBusinessFacts, owner-only API, immutable версии и раздел «Создание» в Console. Design/Developer переиспользуются детерминированно, Content/QA — через guarded Router. Migration 007 добавлена; migration 006 отложена и не применяется. Revise, preview, page generation и публикация не реализованы. [Архитектура, лимиты, API и ограничения](BLOCK-GENERATION.md). Ранее описанные checkpoint ниже — история.
