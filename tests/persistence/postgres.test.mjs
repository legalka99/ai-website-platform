...
test('clean migration creates all foundation tables and repeated migration is a no-op',async()=>{
 ...
 for(const name of ['users','organizations','memberships','projects','workflow_runs','agent_executions','domain_snapshots','websites','website_versions','qa_reports','ai_usage','approvals','audit_events','schema_migrations','project_briefs','owner_commands'])assert.ok(tables.rows.some(r=>r.table_name===name));
