...
import { runDesignSmoke, parseDesignSmokeArgs, designSmokeOutput } from '../scripts/smoke-design.mjs';
import { validOutputs } from './fixtures/website.mjs';
const key='TEST_ONLY_DESIGN_SMOKE_KEY';
...
const wire=()=>({...validOutputs().design,notes:null,visualReferences:null,colors:{...validOutputs().design.colors,secondary:null,accent:null}});
for(const provider of ['openai','yandex']) {
 test(`Design smoke parses ${provider}`,()=>assert.deepEqual(parseDesignSmokeArgs([`--provider=${provider}`]),{provider,confirmed:false}));
 test(`Design smoke ${provider} opt-in uses actual service/router/adapter with offline transport`,async()=>{
  let calls=0;const lines=[];
  const exit=await runDesignSmoke([`--provider=${provider}`,'--confirm-paid-request'],{loadEnvironment:()=>env,write:s=>lines.push(s),transport:async(_url,init)=>{
    calls++;const request=JSON.parse(init.body);assert.equal(request.max_tokens??request.max_output_tokens,2000);
    const schema=request.text?.format.schema??request.response_format.json_schema.schema;assert.ok(schema.properties.styleName);
  ...
  assert.deepEqual(output.design,validOutputs().design);assert.equal(output.provider,provider);assert.equal(output.routing.attempts.length,1);
  ...
  assert.equal(output.usage.actorId,'local-owner');assert.equal(output.usage.organizationId,'local');assert.equal(output.usage.agentType,'design');
...
test('Design smoke opt-in gate precedes env/config/transport access',async()=>{
 let accessed=0;const exit=await runDesignSmoke([],{loadEnvironment:()=>{accessed++;throw Error();},transport:async()=>{accessed++;throw Error();},write:()=>{}});
...
for(const args of [['--provider=unknown','--confirm-paid-request'],['--provider=openai','--provider=yandex','--confirm-paid-request'],['--allow-fallback','--confirm-paid-request']]) test('invalid smoke args rejected before environment/network',async()=>{
 let accessed=0;const exit=await runDesignSmoke(args,{loadEnvironment:()=>{accessed++;return env;},write:()=>{},transport:async()=>{accessed++;throw Error();}});
 assert.equal(exit,1);assert.equal(accessed,0);
});
test('malformed smoke config gives controlled error without provider call or secret output',async()=>{
 const lines=[];let calls=0;
 const exit=await runDesignSmoke(['--confirm-paid-request'],{loadEnvironment:()=>({...env,KLEO_AI_TIMEOUT_MS:'bad'}),write:s=>lines.push(s),transport:async()=>{calls++;throw Error(key);}});
...
test('projection omits arbitrary sensitive metadata and detaches repeated references',()=>{
 ...
 const result={success:true,output:validOutputs().design,execution:{usage,goal:'PRIVATE_PROMPT',routing:{decision:{provider:'openai',model:'test-model',fallbackProviders:[],headers:usage.headers},attempts:[{provider:'openai',outcome:'success',usage}]},budget:{requests:1,prompt:'PRIVATE_PROMPT'}}};
 const output=designSmokeOutput(result,{projectId:'project-1',workflowId:'run-1'},'openai','test-model',[key]);
...
test('malformed output is never displayed as a DesignDirection',()=>{
 const output=JSON.parse(designSmokeOutput({success:true,output:{notes:'PRIVATE_RAW'}},{projectId:'p',workflowId:'w'},'openai','test-model'));
 assert.equal(output.success,false);assert.equal(output.design,undefined);
});
test('smoke transient error has one bounded attempt and sanitized diagnostics',async()=>{
 let calls=0;const lines=[];
 const exit=await runDesignSmoke(['--provider=yandex','--confirm-paid-request'],{loadEnvironment:()=>env,write:s=>lines.push(s),transport:async()=>{calls++;return new Response(key,{status:503});}});
...
test('cached token projection permits only nonnegative integer counts through redaction',()=>{
 for(const value of ['PRIVATE_SECRET',-1,1.5]) {
  const output=designSmokeOutput({success:false,execution:{usage:{cachedInputTokens:value}}},{projectId:'p',workflowId:'w'},'openai','test-model');
